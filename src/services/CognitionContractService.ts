...
export function createUnderstanding(params: {
  ...
  return {
    ...
    sourceSignalId: createId('legacy-signal'),
    signalType: 'review_timing_preference',
    ...
    correctionHistory: [],
    reviewHistory: [],
    materialEvidenceSignature: `${params.ruleId}|${params.ruleVersion}|${[...params.evidenceIds].sort().join(',')}|${params.statement.trim().toLowerCase()}`,
...
export function normalizeUnderstanding(raw: unknown): OperatorUnderstanding | null {
  ...
  return {
    ...
    sourceSignalId: String(obj['sourceSignalId'] ?? `legacy-signal-${String(obj['id'])}`),
    signalType: (obj['signalType'] as OperatorUnderstanding['signalType']) ?? 'review_timing_preference',
    sourceSignalStatus: (obj['sourceSignalStatus'] as OperatorUnderstanding['sourceSignalStatus']) ?? 'proposed',
    evidenceIds: (Array.isArray(obj['evidenceIds']) ? obj['evidenceIds'] : []).map(String),
    evidenceCount: Number(obj['evidenceCount'] ?? (Array.isArray(obj['evidenceIds']) ? obj['evidenceIds'].length : 0)),
    ...
    reviewHistory: Array.isArray(obj['reviewHistory']) ? obj['reviewHistory'] as OperatorUnderstanding['reviewHistory'] : [],
    materialEvidenceSignature: String(obj['materialEvidenceSignature'] ?? `${String(obj['ruleId'])}|${String(obj['ruleVersion'])}|${(Array.isArray(obj['evidenceIds']) ? obj['evidenceIds'] : []).map(String).sort().join(',')}|${String(obj['statement']).trim().toLowerCase()}`),
    personalizationEligible: Boolean(obj['personalizationEligible'] ?? (obj['state'] === 'operator_confirmed')),
